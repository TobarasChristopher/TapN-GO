...
async function passwordAuth(){
    ...
    if(spassword == passwords){                 //if match
        location.href = 'new_url';              //relocate to this url
    }
    else{                                       //else
        document.getElementById("status").innerHTML = "Wrong Password!";        //Change P1 id to wrong password
        document.getElementById("status").style.color = "red";                  //Color
    }
...
async function updateColor(){
    var newColor = document.getElementsByName("colorUpdate")[0].value;  //grab color from text field
    if(newColor.length==6){                         //check if hex color is exactly 6 long
        const { data, error } = await supabase      //start a query
            .from('venue')                          //from venue...
            .update({'pColor': newColor})           //...update pColor column to newColor...
            .eq('venueID', venueID)                 //...where venueID is due
        document.getElementById("colorstatus").innerHTML = "Submitted!"; //Confirmation
        document.getElementById("colorstatus").style.color = "blue"; 
        window.location.reload();   
    }
    else{
        document.getElementById("colorstatus").innerHTML = "Invalid Color!";    //Error message saying its invalid
